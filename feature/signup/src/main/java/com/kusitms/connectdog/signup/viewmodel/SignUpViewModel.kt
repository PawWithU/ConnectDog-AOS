...
class SignUpViewModel
    ...
    constructor(
        ...
        private fun checkValidPhoneNumber() =
            intent {
                ...
                val isValid =
                    if (phone.isEmpty()) {
                        ...
                        Regex("^01[016789]\\d{8}$").matches(phone)
                    }
        ...
        fun onPhoneCertificationButtonClick(
            ...
            if (!state.isSendPhoneAuthCode) {
                ...
                if (!state.isDuplicatedPhoneNumber) {
                    sendPhoneAuthCode(onSendMessageClick)
                } else {
                    intent { postSideEffect(SignUpSideEffect.NavigateTo) }
                }
            } else {
                checkPhoneAuthCode(onVerifyCodeClick, state.userType)
            }
        }
        ...
        private fun getIntermediatorPhoneNumberDuplication() =
            viewModelScope.launch {
        ...
        private fun enableSendPhoneAuthCodeButton() =
            intent {
                if (!state.isSendPhoneAuthCode &&
...
//        }
        ...
        private fun updatePhoneCertificationButtonText() = intent { reduce { state.copy(phoneCertificationButtonText = "인증 확인") } }
        ...
        fun onEmailAuthCodeChanged(authCode: String) =
            intent {
                if (authCode.length <= 8) reduce { state.copy(inputEmailAuthCode = authCode) }
                enableEmailCertification()
            }

        private fun enableEmailCertification() =
            intent {
                if (!state.isSendEmailAuthCode && state.isValidEmail == true) {
                    reduce {
                        state.copy(enableEmailCertification = true)
                    }
                } else if (state.isSendEmailAuthCode && state.inputEmailAuthCode.length == 8) {
                    reduce {
                        state.copy(enableEmailCertification = true)
                    }
                } else {
                    reduce { state.copy(enableEmailCertification = false) }
                }
            }

        fun onEmailCertificationButtonClick() {
            if (!state.isSendEmailAuthCode) {
                getEmailAuthCode()
            } else {
                checkEmailAuthCode()
            }
        ...
        private fun checkEmailAuthCode() =
            intent {
        ...
        private fun updateEmailCertificationButtonText() = intent { reduce { state.copy(emailCertificationButtonText = "인증 확인") } }
        ...
        fun onPasswordRegisterNextButtonClick() =
            intent {
                when (state.userType) {
        ...
        private fun enablePasswordRegisterButton() =
            intent {
        ...
        fun onCheckNicknameDuplicationButtonClick() =
            viewModelScope.launch {
        ...
        fun updateProfileImageIndex(index: Int) = intent { reduce { state.copy(profileImageId = index) } }

        private fun enableNicknameDuplicationButton() =
            intent {
                if (state.nickname.length >= 2 && state.isAvailableNickName) {
                    reduce {
                        state.copy(
                    ...
                    reduce { state.copy(enableNicknameDuplication = false) }
                }
            }

        private fun getNameDuplication() =
            viewModelScope.launch {
                getIntermediatorNameDuplicationUseCase(state.name).onSuccess {
                }
            }

        private fun initNormalVolunteerSignUp() =
            viewModelScope.launch {
        ...
        private fun initSocialVolunteerSignUp() =
            viewModelScope.launch {
        ...
        private fun setNormalVolunteerLogin() =
            viewModelScope.launch {
        ...
        private fun setSocialVolunteerLogin() =
            viewModelScope.launch {
//
            }
    }
