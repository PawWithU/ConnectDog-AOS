...
@HiltViewModel
class EmailAuthForPasswordResetViewModel @Inject constructor(
    ...
    private fun sendAuthCode(userType: UserType) = when (userType) {
        UserType.INTERMEDIATOR -> getIntermediatorEmailAuthCode()
        else -> getVolunteerEmailAuthCode()
    }

    private fun getVolunteerEmailAuthCode() = viewModelScope.launch {
    ...
    private fun getIntermediatorEmailAuthCode() = viewModelScope.launch {
